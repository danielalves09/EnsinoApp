...
✅ Pré-requisitos
.NET 9 SDK
SQL Server 2019+ ou SQL Server via Docker
Docker (opcional, para rodar tudo em container)
...
(sem login) Landing page, inscrição online, validação de certificado
...
Uma Razor View (`CertificadoTemplate.cshtml`) é renderizada para HTML em memória via `RazorViewToStringRenderer`
O HTML é convertido para PDF pelo `SynchronizedConverter` (DinkToPdf)
O PDF é salvo em `wwwroot/Certificados/` e disponibilizado para download em lote via ZIP
Um código de validação único (10 caracteres alfanuméricos) é gerado e salvo na matrícula
Um QR Code aponta para `{CertificadoBaseUrl}{CodigoValidacao}`
Qualquer pessoa pode validar o certificado em `/certificado/validar/{codigo}`
